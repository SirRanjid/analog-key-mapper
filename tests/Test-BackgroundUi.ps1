...
if ($env:OS -ne 'Windows_NT') { throw 'The background UI harness requires Windows.' }
$workspace = Split-Path -Parent $PSScriptRoot
$testFolder = Join-Path $PSScriptRoot ('synthetic-background-ui-' + [Guid]::NewGuid().ToString('N'))
[void][System.IO.Directory]::CreateDirectory($testFolder)
$testProcess = $null; $success = $false
try {
    $compiler = Join-Path $env:WINDIR 'Microsoft.NET\Framework64\v4.0.30319\csc.exe'
    $executable = Join-Path $testFolder 'BackgroundUiHarness.exe'
    ...
    $arguments = @('/nologo','/target:exe','/platform:x64','/langversion:5','/optimize+','/warnaserror+','/main:Tk75.Tests.BackgroundUiHarness',
    ...
    $arguments += $shared; $arguments += $appSources; $arguments += (Join-Path $PSScriptRoot 'BackgroundUiHarness.cs')
    & $compiler @arguments
    if ($LASTEXITCODE -ne 0) { throw 'Background UI harness compilation failed.' }
    ...
    if (-not $testProcess.WaitForExit(30000)) {
        ...
        throw ('Background UI harness exceeded 30 seconds. Artifacts: ' + $testFolder)
    }
    $testProcess.Refresh()
    ...
    if ($testProcess.ExitCode -ne 0) { throw ('Background UI harness failed. Artifacts: ' + $testFolder) }
    $success = $true
    ...
    if ($success -and -not $KeepArtifacts) {
        ...
        if (-not $resolved.StartsWith($parent,[StringComparison]::OrdinalIgnoreCase) -or -not [System.IO.Path]::GetFileName($resolved).StartsWith('synthetic-background-ui-')) { throw 'Refusing cleanup outside the dedicated test directory.' }
