...
$artifactDirectory = Join-Path $workspace ('build\documentation-screenshots-' + [Guid]::NewGuid().ToString('N'))
...
$arguments = @('/nologo','/target:exe','/platform:x64','/langversion:5','/optimize+','/warnaserror+','/main:Tk75.Tests.DocumentationScreenshots',
...
$data = Join-Path $artifactDirectory 'synthetic-data'; $images = Join-Path $workspace 'docs\images'
$process = $null
try {
