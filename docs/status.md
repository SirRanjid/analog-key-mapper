...
The rc.6 revision separates individual key ranges from the global scale, adds vertical behavior controls and calibration, curve-setting sliders, threshold annotations, themed native controls, and visible shutdown phases. Known RGB states need fewer verification requests; initial backup discovery still reads the complete original picture twice. The [release page](https://github.com/SirRanjid/analog-key-mapper/releases) identifies each exact build and its [automated validation](https://github.com/SirRanjid/analog-key-mapper/actions); the recorded results below remain tied to their stated versions.
