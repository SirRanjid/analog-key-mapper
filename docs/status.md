...
The combined revision passed **16,993 local application UI assertions**, including **230 key-selection checks**, **2,373 scrollbar interaction checks**, **354 redraw checks** and **778 native theme checks**. Selection checks compare actual control, native handle, row and cell identity, then verify updated values and scoped edits through real keyboard-selection events. Scrollbar checks inspect existing pixels during hover and held dragging, confirm real panel/table/list/text/popup scrolling, and cover 32-bit positions, arrow/page repetition, focus, capture loss, changed ranges and handle recreation. These are synthetic controls and inputs, without real hardware or an OS shutdown.

The local UI run used Windows' **Show window contents while dragging** option enabled. A separate isolated build exercised the disabled branch and passed **2,375 checks**: the thumb moves while held, content stays in place until release, and release commits the new content position. It substituted the preference read in temporary source copies; actual Windows settings and pointer position were unchanged. Assertion totals can vary with native focus and deferred-drag paths. Earlier background/tray and shutdown runs passed **205** and **101** assertions; the release workflow reruns these suites on the downloadable revision.
