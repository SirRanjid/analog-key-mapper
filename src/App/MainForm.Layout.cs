    ...
    public sealed partial class MainForm
    {
        ...
        void BuildUi()
        {
            ...
            helpPage.Controls.Add(new TextBox { Dock = DockStyle.Fill, ReadOnly = true, Tag = "localized", Multiline = true, ScrollBars = ScrollBars.Vertical, BackColor = ModernTheme.Surface, Text = Tr(
                ...
                "Mehrere Controller\r\n\r\nJeder Controller hat eigene Zuordnungen und einen eigenen USB-Anschluss. Klicke auf Stecker oder Buchse oder ziehe den Stecker hinein. Die kleinen Anschlüsse in der Fußleiste bedienen jeden Controller direkt; dort ziehst du vertikal. „Alle aus“ trennt alle Controller; eine zusätzliche Tastenkombination ist optional. Der neue Xbox-Ausgabeweg unterstützt zunächst einen verbundenen Controller. Weitere Controller lassen sich bereits einrichten und abwechselnd verwenden. Eigene Namen gelten in dieser App; der Gerätename in Windows bleibt beim vorgesehenen Xbox-Ausgabeweg fest.\r\n\r\n" +
                "Aktueller Stand\r\n\r\nDie Druckeingabe wurde an der angeschlossenen TK75 geprüft. Der Xbox-Ausgabeweg hat den Windows-Start- und Abschalttest bestanden und ist zum praktischen Test verfügbar. PS5-Ausgabe ist noch nicht bereit. Der Anschluss zeigt daher erst dann Verbunden, wenn der Ausgabeweg die Verbindung bestätigt. Verbindungsfehler stehen unten. Profile und eigene Kalibrierungen werden getrennt im Unterordner data gespeichert.",
                ...
                "Multiple controllers\r\n\r\nEach controller has its own mappings and USB connection. Click the plug or socket, or slide the plug in. The small footer connectors address each controller directly; drag vertically there. “All off” disconnects every controller; an additional shortcut is optional. The new Xbox output currently supports one connected controller. Additional controllers can be configured and used in turn. Custom names apply within this app; the planned Xbox backend keeps the device name in Windows fixed.\r\n\r\n" +
                "Current status\r\n\r\nPressure input has been checked on the connected TK75. Xbox output passed the Windows startup and shutdown test and is available for practical testing. PS5 output is not ready yet. The connector shows Connected only after the output backend confirms a connection. Connection errors appear at the bottom. Profiles and custom calibrations are stored separately in the data subfolder.") });
        ...
        ContextMenuStrip BuildMainMenu()
        {
            ...
            menu.Items.Add("Profil & Programmregeln", null, delegate { ShowPage("behavior"); }); menu.Items.Add("Verbindung & Entwicklungsstand", null, delegate { ShowPage("help"); }); menu.Items.Add(new ToolStripSeparator()); AddLanguageMenu(menu); StyleMenu(menu); return menu;
        }
