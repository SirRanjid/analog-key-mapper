...
namespace Tk75.App
{
    ...
    public sealed class CalibrationDialog : Form
    {
        ...
        readonly ReaderSession reader;
        readonly int index;
        readonly Label status = new Label { Dock = DockStyle.Fill, Padding = new Padding(16) };
        readonly CheckBox full = new CheckBox { Text = "Ich habe diese Taste in beiden Durchgängen bis zum Anschlag gedrückt.", AutoSize = true };
        readonly TextBox travel = new TextBox { Width = 100 };
        readonly Timer timer = new Timer { Interval = 100 };
        readonly ProgressBar progress = new SleekProgressBar { Dock = DockStyle.Fill, Height = 12, Maximum = 2 };
        readonly Label values = new Label { Dock = DockStyle.Fill, AutoEllipsis = true, TextAlign = ContentAlignment.MiddleLeft, Tag = "muted" };
        readonly Button save = new SleekButton { Text = "Übernehmen", AutoSize = true, Enabled = false, Tag = "primary" };
        KeyLearner learner;
        bool recording;
        int minimum = int.MaxValue, maximum = int.MinValue, samples, last;
        readonly HashSet<int> distinct = new HashSet<int>();
        string error;
        public CalibrationEntry Result { get; private set; }
        public CalibrationDialog(ReaderSession input, int keyIndex, string label, CalibrationEntry existing)
        {
            UiText.PreserveText(this); UiText.PreserveText(status); UiText.PreserveText(values);
            reader = input; index = keyIndex; Text = UiText.Get("Taste kalibrieren · ", "Calibrate key · ") + label; ClientSize = new Size(640, 390); StartPosition = FormStartPosition.CenterParent;
            FormBorderStyle = FormBorderStyle.FixedDialog; MaximizeBox = false; MinimizeBox = false;
            var layout = new TableLayoutPanel { Dock = DockStyle.Fill, RowCount = 7, ColumnCount = 1, Padding = new Padding(22, 14, 22, 14) };
            layout.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
            layout.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 16)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 34)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 54)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 24)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 0)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 50)); Controls.Add(layout); layout.Controls.Add(status, 0, 0);
            status.Padding = new Padding(0, 12, 0, 8);
            layout.Controls.Add(progress, 0, 1); layout.Controls.Add(values, 0, 2);
            full.AutoSize = false; full.Dock = DockStyle.Fill; full.Text = "Ich habe die Taste beide Male bis zum Anschlag gedrückt."; layout.Controls.Add(full, 0, 3);
            var detail = new LinkLabel { Text = "Optional: gemessenen Tastenhub angeben", Dock = DockStyle.Fill };
            var measured = new FlowLayoutPanel { Dock = DockStyle.Fill, Visible = false }; measured.Controls.Add(new Label { Text = "Selbst gemessener Gesamthub (mm)", AutoSize = true, Padding = new Padding(0, 5, 0, 0) }); measured.Controls.Add(travel); layout.Controls.Add(detail, 0, 4); layout.Controls.Add(measured, 0, 5);
            detail.LinkClicked += delegate { measured.Visible = !measured.Visible; layout.RowStyles[5].Height = measured.Visible ? 38 : 0; ClientSize = new Size(640, measured.Visible ? 428 : 390); };
            if (existing != null && existing.MeasuredTravel.HasValue) travel.Text = existing.MeasuredTravel.Value.ToString(CultureInfo.InvariantCulture);
            var bar = new FlowLayoutPanel { Dock = DockStyle.Fill }; var restart = new SleekButton { Text = "Neu aufnehmen", AutoSize = true }; var cancel = new SleekButton { Text = "Abbrechen", AutoSize = true, DialogResult = DialogResult.Cancel }; bar.Controls.Add(save); bar.Controls.Add(restart); bar.Controls.Add(cancel); layout.Controls.Add(bar, 0, 6); CancelButton = cancel;
            restart.Click += delegate { BeginRecording(); };
            full.CheckedChanged += delegate { lock (gate) save.Enabled = CanSave() && full.Checked; };
            ModernTheme.Apply(this); UiText.Apply(this);
            save.Click += delegate
            {
                try
                {
                    ...
                    {
                        if (!CanSave())
                            throw new InvalidOperationException("Es fehlen zwei vollständige, eindeutige Druck-/Loslasszyklen mit genügend unterschiedlichen Messwerten.");
                        if (!full.Checked) throw new InvalidOperationException("Den tatsächlichen Anschlag bestätigen. Ein beobachtetes Maximum allein beweist keinen vollständigen Tastendruck.");
                        double? mm = string.IsNullOrWhiteSpace(travel.Text) ? (double?)null : double.Parse(travel.Text.Replace(',', '.'), CultureInfo.InvariantCulture);
                        Result = new CalibrationEntry { KeyIndex = index, Rest = minimum, Bottom = maximum, UsableMin = minimum, UsableMax = maximum, MeasuredTravel = mm };
                        var errors = MappingValidation.ValidateCalibration(Result.Value()); if (errors.Count != 0) throw new InvalidOperationException(string.Join(" ", errors.ToArray()));
                        DialogResult = DialogResult.OK; Close();
                    }
                }
                catch (Exception ex) { MessageBox.Show(this, UiText.Get(ex.Message), UiText.Get("Kalibrierung")); }
            };
            Shown += delegate { reader.Sample += Feed; BeginRecording(); timer.Start(); };
            FormClosed += delegate { reader.Sample -= Feed; timer.Stop(); timer.Dispose(); };
            timer.Tick += delegate
            {
                lock (gate)
                {
                    if (!reader.IsReading) { error = reader.Status; recording = false; }
                    if (learner == null) return;
                    progress.Value = learner.ReleasedCycles;
                    values.Text = samples == 0 ? UiText.Get("Noch keine Messwerte für ", "No readings yet for ") + label : String.Format(UiText.Get("Messwert {0}   ·   Bisheriger Bereich {1}–{2}", "Reading {0}   ·   Observed range {1}–{2}"), last, minimum, maximum);
                    save.Enabled = CanSave() && full.Checked;
                    string instruction = learner.State == KeyLearnerState.Completed ? UiText.Get(CanSave() ? "Fertig. Anschlag bestätigen und übernehmen." : "Zu wenige unterschiedliche Druckwerte. Bitte langsamer neu aufnehmen.") :
                        learner.State == KeyLearnerState.Pressing ? UiText.Get("Bis zum Anschlag drücken, dann vollständig loslassen.") :
                        learner.State == KeyLearnerState.Armed ? UiText.Get("Noch einmal langsam bis zum Anschlag drücken und loslassen.") :
                        String.Format(UiText.Get("Jetzt nur {0} langsam bis zum Anschlag drücken und vollständig loslassen.", "Slowly press only {0} all the way down, then release fully."), label);
                    status.Text = error != null ? UiText.Get(error) : String.Format(UiText.Get("{0} · {1} von 2 Durchgängen abgeschlossen\n\n{2}", "{0} · {1} of 2 passes completed\n\n{2}"), label, learner.ReleasedCycles, instruction) + (samples == 0 ? "\n" + UiText.Get("Du bestimmst das Tempo.") : "");
                }
            };
        }
        void BeginRecording()
        {
            lock (gate)
            {
                learner = new KeyLearner(); minimum = int.MaxValue; maximum = int.MinValue;
                samples = 0; last = 0; distinct.Clear(); error = null; recording = reader.IsReading;
                full.Checked = false; save.Enabled = false;
                if (!recording) error = reader.Status;
            }
        }
        bool CanSave()
        {
            return reader.IsReading && error == null && learner != null && learner.State == KeyLearnerState.Completed && learner.KeyIndex == index &&
                samples >= 10 && distinct.Count >= 5 && minimum == 0 && maximum > minimum && last == 0;
        }
        void Feed(TravelSample sample, double elapsed)
        {
            ...
            {
                if (!recording) return;
                if (sample.KeyIndex != index && sample.RawValue > 0) { error = "Eine andere Taste wurde gedrückt. Aufnahme neu starten."; recording = false; return; }
                if (sample.KeyIndex != index) return;
                samples++; distinct.Add(sample.RawValue); last = sample.RawValue; minimum = Math.Min(minimum, last); maximum = Math.Max(maximum, last); learner.Feed(sample, elapsed);
                if (learner.State == KeyLearnerState.Ambiguous) { error = learner.Error; recording = false; }
                if (learner.State == KeyLearnerState.Completed) recording = false;
            }
        }
    }
